...
from filter.video_processor import VideoProcessor
...
class DiffProcessor:
    ...
    def get_section(dataset):
        ...
        config.read('preprocess/diff_config.ini')
...
class PixelDiff(DiffProcessor):
    ...
    def cal_frame_diff(self, frame, prev_frame):
        ...
        frame_diff = cv2.threshold(frame_diff, self.pixel_thresh_low_bound, 255, cv2.THRESH_BINARY)[1]
...
class AreaDiff(DiffProcessor):
    ...
    def get_frame_feature(self, frame):
        ...
        blur = cv2.GaussianBlur(gray, (self.area_blur_rad, self.area_blur_rad), self.area_blur_var)
        return blur
    ...
    def cal_frame_diff(self, frame, prev_frame):
        ...
        thresh = cv2.threshold(frame_delta, self.area_thresh_low_bound, 255, cv2.THRESH_BINARY)[1]
...
class EdgeDiff(DiffProcessor):
    ...
    def get_frame_feature(self, frame):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(gray, (self.edge_blur_rad, self.edge_blur_rad), self.edge_blur_var)
    ...
    def cal_frame_diff(self, edge, prev_edge):
        ...
        frame_diff = cv2.cvtColor(frame_diff, cv2.COLOR_BGR2GRAY)
        frame_diff = cv2.threshold(frame_diff, self.edge_thresh_low_bound, 255, cv2.THRESH_BINARY)[1]
        changed_pixels = cv2.countNonZero(frame_diff)
        fraction_changed = changed_pixels / total_pixels
        return fraction_changed
...
class HistDiff(DiffProcessor):
    ...
    def get_frame_feature(self, frame):
        ...
        for i in range(nb_channels):
            hist[i * self.hist_nb_bins: (i + 1) * self.hist_nb_bins] = cv2.calcHist(frame, [i], None, [self.hist_nb_bins], [0, 256])
...
class HOGDiff(DiffProcessor):
    ...
    def get_frame_feature(self, frame):
        ...
        hog = skimage.feature.hog(gray, orientations=self.hog_orientations,
